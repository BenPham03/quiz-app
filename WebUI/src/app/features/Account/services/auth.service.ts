...
export class AuthService {
  ...
  loginWithGoogle(credencial: string): Observable<LoginResponse> {
    ...
    let a = this.http.post<LoginResponse>(
      `${Base_URL}/Authentication/google-login`,
  ...
  sendMail(sendEmailRequest: SendEmailRequest): Observable<any> {
    ...
    return this.http.post(`${Base_URL}/Mail/send-email`, sendEmailRequest, {
      headers,
    });
  }
  ...
  updatePassword(
    ...
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.post(
      `${Base_URL}/Authentication/update-password`,
  ...
  editTimeline(timeline: string): Observable<any> {
    const params = { timeline };
    return this.http.put(`${Base_URL}/Authentication/edit-timeline`, null, {
      params,
    });
  }
}
