...
export class ReportComponent implements OnInit {
  ...
  loadChart(): void {
    this.earliestEntry = this.Analyst.reduce((earliest, current) =>
      current.attemptAt.getTime() < earliest.attemptAt.getTime()
        ? current
        : earliest
    );
    this.latestEntry = this.Analyst.reduce((latest, current) => {
      const currentTimeWithDuration =
        current.attemptAt.getTime() + current.duration * 1000; // Cộng duration (sang ms)
      const latestTimeWithDuration =
        latest.attemptAt.getTime() + latest.duration * 1000; // Cộng duration (sang ms)

      return currentTimeWithDuration > latestTimeWithDuration
        ? current
        : latest;
    });
    this.chartWidth = this.calculateMinutesDifference(
      this.earliestEntry.attemptAt.getTime(),
      this.latestEntry.attemptAt.getTime() + this.latestEntry.duration * 1000
    );
